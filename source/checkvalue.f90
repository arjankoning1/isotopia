...
200 if (Atarget ==  -1) then
  ...
  if (Zdepth < 0 .or. Zdepth > Ztarget - 1) then
    write(*, '(" ISOTOPIA-error: 0 < = Zdepth < = Ztarget - 1")')
    stop
  endif
  if (Adepth < 0 .or. Adepth > Atarget - 1) then
    write(*, '(" ISOTOPIA-error: 0 < = Adepth < = Atarget - 1")')
    stop
  endif
